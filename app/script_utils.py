...
def send_balance_due_soon_notification():
    ...
    accounts = LoanAccount.objects.filter(
        status=LoanAccount.ACTIVE, balance__gt=0, due_date=two_days_from_now
    ).select_related("user")
    msg_out = 0
    for account in accounts:
        WhatsAppClient.send_balance_due_soon_message(
            account.user.whatsapp_number,
...
def send_balance_due_warning_today_notification():
    today = date.today()
    accounts = LoanAccount.objects.filter(
        status=LoanAccount.ACTIVE, balance__gt=0, due_date=today
    ).select_related("user")
    msg_out = 0
    for account in accounts:
        WhatsAppClient.send_balance_due_warning_message(
            account.user.whatsapp_number,
            account.user.full_name,
            account.total,
            account.due_date,
...
def send_balance_past_due_notification():
    today = date.today()
    accounts = LoanAccount.objects.filter(
        status=LoanAccount.ACTIVE, balance__gt=0
    ).select_related("user")
    msg_out = 0
    for account in accounts:
        WhatsAppClient.send_past_due_message(
            account.user.whatsapp_number,
            account.user.full_name,
            account.total,
            account.due_date,
